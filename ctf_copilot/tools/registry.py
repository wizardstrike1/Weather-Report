...
SPECS: list[ToolSpec] = [
    ...
    ToolSpec("mediainfo", Category.FORENSICS, "mediainfo",
             _t("mediainfo", "{file}"),
             "Container/codec/stream metadata for audio & video",
             {"linux": "apt install mediainfo"}),
    ToolSpec("zbarimg", Category.FORENSICS, "zbarimg",
             _t("zbarimg", "-q", "{file}"),
             "Decode QR / barcodes from an image",
             {"linux": "apt install zbar-tools"}),
    ToolSpec("multimon-ng", Category.FORENSICS, "multimon-ng",
             _t("multimon-ng", "-a", "{arg}", "{file}"),
             "Decode DTMF/Morse/AFSK tones (prefer the built-in 'tones' tool "
             "which feeds it correctly via ffmpeg)",
             {"linux": "apt install multimon-ng"}),
